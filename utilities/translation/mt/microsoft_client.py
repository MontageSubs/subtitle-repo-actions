...
CUE_MARKER_PATTERN = re.compile(r"\u27e6c(\d+(?:\.\d+)?)\u27e7", re.IGNORECASE)

CORRUPT_MARKER_PATTERN = re.compile(r"\\+[^\u27e6\u27e7]{0,6}?(\d{1,6}(?:\.\d{1,6})?)\u27e7")
...
CORRUPT_MARKER_SIGNATURE = re.compile(
    r"\\+[^\u27e6\u27e7]{0,6}?\d{1,6}(?:\.\d{1,6})?\u27e7|" + UNCLOSED_MARKER_SIGNATURE + "|" + MISSING_OPEN_MARKER_SIGNATURE
)
...
def parse_translated_html(html, marker_pattern, prefix_char=None, expected_ids=None):
    ...
    raw = split_by_marker(flat, marker_pattern)
    if prefix_char == "c":
        marker_key_pattern = re.compile(r"^\d+(?:\.\d+)?$")
        return {k.strip(): v for k, v in raw.items() if marker_key_pattern.match(k.strip())}
    return {int(k.strip()): v for k, v in raw.items() if k.strip().isdigit()}
...
def run_packed_jobs_with_lookahead(primary, speculative, max_chars_per_request, lang, target_lang, concurrency):
    ...
    def process_chunk(ci, chunk_idx_list):
        ...
        chunk_payloads = [primary[sid] if kind == "primary" else speculative[sid] for sid, kind in items]
        try:
            resp = call_microsoft_api(chunk_payloads, lang.current(), target_lang)
            if resp and isinstance(resp, list):
                for i, r_item in enumerate(resp):
                    if i < len(items) and r_item.get("translations"):
                        sid, kind = items[i]
                        (primary_results if kind == "primary" else speculative_results)[sid] = r_item["translations"][0]["text"]
        except Exception as e:
            log(f"packed jobs chunk failed: {e}")
...
def build_window_job(units, index, suspect_id, radius, batch_chars):
...
def retry_windowed_all(units, suspect_ids, lang, target_lang, batch_chars, concurrency, ladder=WINDOW_RADIUS_LADDER, strict_marker=False):
    ...
    for ladder_index, radius in enumerate(ladder):
        ...
        for suspect_id in active:
            job = build_window_job(units, index, suspect_id, radius, batch_chars)
            if job:
                jobs[suspect_id] = job
        if not jobs:
            continue
...
def retry_isolated_cues_all(missing_by_unit, marker_order, marker_text_by_id, marker_term_matches, lang, target_lang, batch_chars, concurrency, extra_valid=None):
...
def translate_units(units, chapters, cues, lang, target_lang, batch_chars, concurrency, raw_context=None, array_size=1):
    ...
    if all_suspects:
        ...
        for uid in sorted(all_suspects):
            ...
            trivial = [cid for cid in remaining if not has_translatable_content(marker_text_by_id.get(cid, ""), marker_term_matches.get(cid))]
            non_trivial = remaining
            if trivial:
                filled = {cid: apply_term_replacements(marker_text_by_id[cid], marker_term_matches.get(cid) or [], target_lang) for cid in trivial}
        ...
        if missing_by_unit:
            recovered_cues = retry_isolated_cues_all(missing_by_unit, marker_order, marker_text_by_id, marker_term_matches, lang, target_lang, batch_chars, concurrency)
