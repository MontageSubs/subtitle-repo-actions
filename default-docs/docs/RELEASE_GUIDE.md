...
一个 **release** 对应一份独立时间轴：剪辑版本、片源或区域任一不同，即建立独立 release 目录，互不影响、互不覆盖。

## 触发方式

通过 Actions 中的 **新增 Release (Add Release)** workflow 手动触发（首次初始化时也会一并要求填写），需在表单中提供以下信息，**三选一，不可多选，不可全部留空**：

- **来源：WEB** —— 勾选表示片源为网络流媒体版本
- **来源：BluRay** —— 勾选表示片源为蓝光版本
- **自定义标识**（文本框）—— 上方两项均不适用时，在此填写完整来源标识；若已勾选 WEB 或 BluRay，此处可选填区域 / 剪辑版本后缀
